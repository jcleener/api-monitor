	...
	ctx.inject(['settings', 'credentials', 'subagents', 'sessions', 'sessionProjections', 'webServer'], (host) => {
		...
		async function pollDeepseek() {
			...
			try {
				...
				if (chosen) { entry.balance = num(chosen.total_balance, 0); entry.currency = chosen.currency; }
		...
		async function pollVolcesCoding() {
			...
			try {
				...
				if (entry.five === null && entry.weekly === null && entry.monthly === null) { entry.available = false; entry.unsubscribed = true; return; }
				entry.available = true;
			} catch (e) { entry.error = '接口错误'; }
		}
