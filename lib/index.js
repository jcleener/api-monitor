...
// - SiliconFlow 按模型公开价（DeepSeek-V4-Flash 预览 ¥1/¥2）。
...
const DEFAULT_PRICES = {
	...
	siliconflow: { in: 1, cr: 0.02, cw: 0, out: 2 },  // 供应商兜底 = SF V4-Flash 预览价
	'deepseek-v4-flash': {
...
// 花费（CNY）。DeepSeek 峰谷定价无法按请求时刻归因（日志无逐事件时间戳），返回 {peak, offPeak}；
// 平铺价 peak === offPeak。未知模型返回 null（界面显示 —）。
...
export function apply(ctx) {
	ctx.inject(['settings', 'credentials', 'subagents', 'sessions', 'sessionProjections', 'webServer'], (host) => {
		...
		async function pollAll() {
			await Promise.all([refreshDeepseekPrices(false), pollDeepseek(), pollSiliconflow(), pollOpencode(), pollVolcesCoding(), pollVolcesAgent()]);
		}
