...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 花费预估（host 已按 CNY 返回；聚合 costs 只有 {peak,offPeak}，用两者差异判断峰谷）
		const fmtCost = (c) => {
			if (!c || c.peak === undefined || !Number.isFinite(c.peak)) return "—";
			const peak = Math.max(0, c.peak), off = Math.max(0, c.offPeak ?? c.peak);
			if (Math.abs(peak - off) > 0.005) return "梁文峰 ¥" + peak.toFixed(2) + " | 梁文谷 ¥" + off.toFixed(2);
		...
		function ApiMonitor(props) {
			...
			if (vis.siliconflow !== false) {
				...
				vrows.push(react.createElement("div", { className: "am-vrow", key: "sf" }, dot(balDot(p.siliconflow)), react.createElement("span", { className: "am-vlbl" }, "SiliconFlow"), react.createElement("span", { className: "am-vval" + (p.siliconflow && p.siliconflow.stale ? " am-stale" : "") }, b !== undefined ? "¥" + b.toFixed(2) : (p.siliconflow && p.siliconflow.error ? "—" : "…"))));
			}
			...
			const body = react.createElement("div", { className: "am-panelbody" },
				...
				react.createElement("div", { className: "am-block" },
					...
					react.createElement("div", { className: "am-row" }, react.createElement("span", { className: "k" }, "会话花费预估"), react.createElement("span", { className: "v" }, fmtCost(session.costs && session.costs.siliconflow))),
					p.siliconflow && p.siliconflow.stale ? react.createElement("div", { className: "am-stale-note" }, "↑ 显示上次数据（" + fmtTs(p.siliconflow.lastGoodTs) + " 获取）") : null,
