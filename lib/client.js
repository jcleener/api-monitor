...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ApiMonitor(props) {
			...
			const currentId = (useSessionsHook ? useSessionsHook((s) => {
				// 「当前会话」。0.1.5 及更早：SessionListState.current。
				// 0.1.6 删除了该字段（present 才静默降级：currentId 恒 null → root 不发出 →
				// 宿主走 getSnapshot(null) 的全零分支，余额照常、token/花费恒 0）。
				// 0.1.6 的口径改由「主视图保留标记」推导，与上游 ui-workspace 的
				// mainSessionId() 同一表达式：retainedBy.mainView > 0 即主视图正在展示它。
				if (!s) return null;
				if (typeof s.current === "string") return s.current;
				const byId = s.byId || {};
				const ids = Array.isArray(s.ids) && s.ids.length ? s.ids : Object.keys(byId);
				for (const id of ids) {
					const r = byId[id] && byId[id].retainedBy;
					if (r && (r.mainView || 0) > 0) return id;
				}
				return null;
			}) : null) || null;
