...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region styles
		const css = [
			".am-vblock{display:flex;flex-direction:column;gap:2px;width:100%;padding:5px 10px;border-radius:9px;box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2,#333);background:var(--dsw-alias-button-ghost-hover,#2a2a2a);cursor:pointer}",
			...
			".am-panel{position:fixed;left:12px;bottom:108px;z-index:21;width:420px;max-width:calc(100vw - 24px);background:var(--dsw-specific-menu,#1e1e1e);border:1px solid var(--dsw-alias-border-inverted,#333);border-radius:12px;box-shadow:var(--dsw-shadow-lv3,0 14px 44px rgba(0,0,0,.5));display:flex;flex-direction:column;max-height:60vh;overflow:hidden}",
			...
			".am-btn{background:var(--dsw-alias-button-ghost-hover,#2a2a2a);border:1px solid var(--dsw-alias-border-l2,#3a3a3a);color:var(--dsw-alias-label-primary,#eee);border-radius:7px;padding:4px 10px;font-size:12px;cursor:pointer}",
			...
			".am-select{background:var(--dsw-alias-button-ghost-hover,#2a2a2a);color:var(--dsw-alias-label-primary,#eee);border:1px solid var(--dsw-alias-border-l2,#3a3a3a);border-radius:7px;padding:3px 6px;font-size:12px}",
			".am-input{background:var(--dsw-alias-button-ghost-hover,#2a2a2a);color:var(--dsw-alias-label-primary,#eee);border:1px solid var(--dsw-alias-border-l2,#3a3a3a);border-radius:7px;padding:4px 6px;font-size:12px;width:150px}",
		...
		function ApiMonitor(props) {
			...
			if (vis.deepseek !== false) {
				...
				vrows.push(react.createElement("div", { className: "am-vrow", key: "ds" }, dot(balDot(p.deepseek)), react.createElement("span", { className: "am-vlbl" }, "DeepSeek"), react.createElement("span", { className: "am-vval" }, b !== undefined ? "¥" + b.toFixed(2) : (p.deepseek && p.deepseek.error ? "—" : "…"))));
			}
			if (vis.siliconflow !== false) {
				const b = p.siliconflow && p.siliconflow.balance;
				vrows.push(react.createElement("div", { className: "am-vrow", key: "sf" }, dot(balDot(p.siliconflow)), react.createElement("span", { className: "am-vlbl" }, "SiliconFlow"), react.createElement("span", { className: "am-vval" }, b !== undefined ? "¥" + b.toFixed(2) : (p.siliconflow && p.siliconflow.error ? "—" : "…"))));
			}
			if (vis.opencodeGo !== false) {
				const oc = p.opencode || {};
				vrows.push(react.createElement("div", { className: "am-vrow", key: "oc" }, dot(pctDot([oc.rolling, oc.weekly, oc.monthly])), react.createElement("span", { className: "am-vlbl" }, "OpenCode Go"), react.createElement("span", { className: "am-vval" }, (oc.rolling !== null || oc.weekly !== null || oc.monthly !== null) ? (pctStr(oc.rolling) + "|" + pctStr(oc.weekly) + "|" + pctStr(oc.monthly)) : (oc.error ? "—" : "…"))));
			}
			if (vis.volcesCoding !== false) {
				const vc = p.volcesCoding || {};
				vrows.push(react.createElement("div", { className: "am-vrow", key: "vc" }, dot(pctDot([vc.five, vc.weekly, vc.monthly])), react.createElement("span", { className: "am-vlbl" }, "火山 Coding"), react.createElement("span", { className: "am-vval" }, (vc.five !== null || vc.weekly !== null || vc.monthly !== null) ? (pctStr(vc.five) + "|" + pctStr(vc.weekly) + "|" + pctStr(vc.monthly)) : (vc.error ? "—" : "…"))));
			}
			if (vis.volcesAgent !== false) {
				const va = p.volcesAgent || {};
				vrows.push(react.createElement("div", { className: "am-vrow", key: "va" }, dot(pctDot([va.five, va.weekly, va.monthly])), react.createElement("span", { className: "am-vlbl" }, "火山 Agent"), react.createElement("span", { className: "am-vval" }, (va.five !== null || va.weekly !== null || va.monthly !== null) ? (pctStr(va.five) + "|" + pctStr(va.weekly) + "|" + pctStr(va.monthly)) : (va.error ? "—" : "…"))));
			}
